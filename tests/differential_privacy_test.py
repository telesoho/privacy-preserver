...
from preserver.differential_privacy import DPLib
...
if __name__ == '__main__':
    ...
    sdf_true = pd.DataFrame(data=[generate_rand_tuple() for _ in range(10)], columns=schema)
    # sdf_false = [[1, 2, 3], (1, 2, 3), 'string', 5.7, 4 + 6j, sdf_true.rdd]
    sdf_false = [[1, 2, 3], (1, 2, 3), 'string', 5.7, 4 + 6j, sdf_true]
    ...
    label2_false = ['yes', 'no', -3, sdf_true, float('-inf'), '']

    label_sdf_false = [pd.DataFrame(
        data=[generate_rand_tuple(choice_list=('male', 'female', 'undefined')) for _ in range(100)], columns=schema),
        pd.DataFrame(
            data=[generate_rand_tuple(choice_list=('yes', 'no', 'can\'t say for sure', 'I don\'t know'))
                  for _ in range(100)], columns=schema),
        pd.DataFrame(data=[generate_rand_tuple(choice_list=('1', 'no')) for _ in range(100)],
                              columns=schema),
        pd.DataFrame(data=[generate_rand_tuple(choice_list=('yes', '2')) for _ in range(100)],
                              columns=schema)]
    set_column_sdf = pd.DataFrame(data=[generate_rand_tuple() for _ in range(100000)], columns=schema)
